...
struct Render2DData {
        ...
        glm::vec4 QuadVertexPositions[4] = {{-1.f, 1.f, 0.f, 1.f},
                                            {-1.f, -1.f, 0.f, 1.f},
                                            {1.f, -1.f, 0.f, 1.f},
                                            {1.f, 1.f, 0.f, 1.f}};

        phmap::flat_hash_map<TextureSlotType, Resources::TextureHandle>
            Textures;
...
void Renderer2D::Init() {
    ...
    s_RenderData.VertBuff =
        Gl::VertexBuffer::Create({{{Gl::ShaderDataType::Float3, "a_Position "},
    ...
    s_RenderData.Shader = Gl::Shader::Create(
        Resources::Engine::Renderer2D.vert, Resources::Engine::Renderer2D.frag);

    s_RenderData.VertArray->AddVertexBuffer(s_RenderData.VertBuff);
    s_RenderData.VertArray->SetIndexBuffer(s_RenderData.IndexBuff);

    s_RenderData.QuadVertexBufferBase =
        new QuadVertex[Render2DData::MaxVertices];
...
void Renderer2D::DrawQuad(const glm::vec2& position, const glm::vec2& size,
                          ...
                          const glm::vec4& tintColor) {
    DrawQuad(glm::vec3{position.x, position.y, 0.f}, size, texture, texCoords,
             tintColor);
}
...
void Renderer2D::DrawQuad(const glm::mat4& transform,
                          Resources::TextureHandle texture,
                          const glm::vec4& tintColor) {
    static constexpr std::array<float, 8> texCoords = {0.f, 1.f, 0.f, 0.f,
                                                       1.f, 0.f, 1.f, 1.f};
...
void Renderer2D::DrawQuad(const glm::mat4& transform,
                          ...
                          const glm::vec4& tintColor) {
    if (s_RenderData.VertexCount > Render2DData::MaxVertices ||
    ...
    for (int i = 0; i < 4; i++) {
        ...
        s_RenderData.QuadVertexBufferPtr->TexCoord = {texCoord[i * 2],
                                                      texCoord[i * 2 + 1]};
...
void Renderer2D::Flush() {
    ...
    s_RenderData.VertBuff->SetData(s_RenderData.QuadVertexBufferBase,
                                   sizeof(uint32_t) * s_RenderData.VertexCount);
    s_RenderData.VertArray->Bind();

    glDrawElements(GL_TRIANGLES, s_RenderData.QuadIndexCount, GL_UNSIGNED_INT,
                   nullptr);
}
