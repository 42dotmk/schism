        ...
        [this](std::error_code ec, asio::ip::tcp::socket soc) {
            if (!ec) {
                ...
                auto id = reinterpret_cast<uint64_t>(
                    client.get());  // hacky way to get a unique id
                SC_CORE_INFO("Connected client {}", id);
                client->AssignId(id);

                m_Clients[id] = client;
                std::weak_ptr<Client> clientWeakPtr = client;
                client->AttachReadCallback(
                    [this, client = std::move(clientWeakPtr)](
                        std::vector<uint8_t>& buffer, std::size_t length) {
...
void Server::HandleClientRead(const std::weak_ptr<Client>& client,
