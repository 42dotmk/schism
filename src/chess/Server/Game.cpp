...
namespace Chess::Net {
Game::Game(std::weak_ptr<Client> player1, std::weak_ptr<Client> player2,
           bool player1Turn)
...
std::shared_ptr<Game> Game::Create(std::weak_ptr<Client> p1,
                                   std::weak_ptr<Client> p2, bool player1Turn) {
    auto p = std::shared_ptr<Game>(
        new Game(std::move(p1), std::move(p2), player1Turn));
    auto id = reinterpret_cast<uint64_t>(
        p.get());  // very hacky way, quick fix for now
    p->m_Id = id;
    return p;
}
...
void Game::MakeMove(const Move& move,
    ...
    if (p1->Id() == m_NextTurnPlayerId) {
        ...
        SC_CORE_ERROR(
            "Game ({}) invalid NextTurnPlayerId, cannot find next turn player",
            m_Id);
    }
}
}  // namespace Chess::Net