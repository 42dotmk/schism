...
GameClient::GameClient(const std::string& host, const std::string& port,
    ...
    : m_GameEventBus{gameEventBus}, m_Socket(m_IoContext) {
...
void GameClient::HandleRead(size_t length) {
    ...
    switch (m_ReadBuffer[0]) {
        case Net::MessageType::ENTER_GAME: {
            auto enterGame =
                Net::UnpackMessage<Net::EnterGame>(m_ReadBuffer, length);
            m_GameEventBus.PostEvent(enterGame);
            break;
        }
        case Net::MessageType::GAME_MOVE: {
            auto gameMove =
                Net::UnpackMessage<Net::GameMove>(m_ReadBuffer, length);
            m_GameEventBus.PostEvent(gameMove);
            break;
        }
        case Net::MessageType::SUCCESSFUL_MOVE: {
            auto successfulMove =
                Net::UnpackMessage<Net::SuccessfulMove>(m_ReadBuffer, length);
