...
Game::Game(
    ...
    Schism::GameEvent::CallbackBus&
        networkSendBus)  // Interface for the boardrenderer would be better
    : m_BoardRenderer{renderer}, m_NetworkSendBus{networkSendBus} {
...
void Game::ProcessInput(Schism::Event& e) {
    switch (e.GetEventType()) {
        case Schism::MouseButtonPressedEvent::GetStaticType(): {
            ...
            Schism::MouseButtonPressedEvent evt =
                dynamic_cast<Schism::MouseButtonPressedEvent&>(e);
            auto& pos = evt.GetPosition();
            auto boardPosition = FindBoardPositionFromCoord(pos.x, pos.y);

            if (boardPosition.x > board.size() ||
                boardPosition.y > board.size()) {
                return;
            }
            ...
            auto selectPiece = [this](const auto& position) {
                ...
                if (!m_State.isWhite) {
                    m_ValidMoves =
                        m_Engine.GetValidMoves(FlipBoardPosition(position));
                } else {
            ...
            if (m_State.pieceSelected) {
                auto& prevPiece = board[m_State.selectedPosition.x]
                                       [m_State.selectedPosition.y];
                auto& currentPiece = board[boardPosition.x][boardPosition.y];

                if (prevPiece.color == currentPiece.color &&
                    IsValidPiece(currentPiece.type)) {
                    selectPiece(boardPosition);
                    return;
                }

                Move m;
                m.piece = board[m_State.selectedPosition.x]
                               [m_State.selectedPosition.y];
            ...
            if (board[boardPosition.x][boardPosition.y].type ==
                PieceType_Blank) {
                return;
            }
