...
namespace Chess {
Engine::Engine() : m_TurnWhite{true} {
    Reset();
}
...
void Engine::UndoLastMove() {
    ...
    m_Board[prevPosition.x][prevPosition.y] =
        m_Board[currentPosition.x][currentPosition.y];
...
bool Engine::MakeMove(const Move& move) {
    ...
    const auto nextPiece =
        m_Board[move.currentPosition.x][move.currentPosition.y];
    ...
    if (move.piece.color == nextPiece.color &&
        nextPiece.type != PieceType_Blank) {
        return false;
    }
    ...
    for (auto& validMovePosition : validMoves) {
        if (validMovePosition == move.currentPosition) {
            ...
            m_Board[move.prevPosition.x][move.prevPosition.y] = {
                PieceType_Blank};
...
inline bool Engine::CheckObstacle(std::vector<Position>& validMoves,
                                  const Piece& myPiece,
                                  const Position& position,
                                  bool canTake) const {
    if (position.y >= m_Board.size() || position.x >= m_Board.size()) {
        return false;
    }
    auto& piece = m_Board[position.x][position.y];

    if (canTake && IsValidPiece(piece.type) &&
...
void Engine::CheckObstacleVertically(std::vector<Position>& validMoves,
                                     const Position& position, uint8_t length,
                                     bool canTake, bool descending) const {
    const auto& myPiece = m_Board[position.x][position.y];

    for (uint8_t y = position.y + 1;
         y <= static_cast<uint8_t>(position.y + (length - 1)); y++) {
    ...
    if (descending) {
        const uint8_t yLimit =
            (length - 1) > position.y
                ? 0
                : static_cast<uint8_t>(position.y - (length - 1));
...
void Engine::CheckObstacleHorizontally(std::vector<Position>& validMoves,
                                       const Position& position, uint8_t length,
                                       bool canTake, bool descending) const

{  // Make this into a single loop, can be optimized
    const auto& myPiece = m_Board[position.x][position.y];
    for (uint8_t x = position.x + 1;
         x <= static_cast<uint8_t>(position.x + (length - 1)); x++) {
    ...
    if (descending) {
        const uint8_t xLimit =
            (length - 1) > position.x
                ? 0
                : static_cast<uint8_t>(position.x - (length - 1));
...
void Engine::CheckObstacleDiagonally(std::vector<Position>& validMoves,
                                     const Position& position) {
    const auto& myPiece = m_Board[position.x][position.y];
    ...
    for (uint8_t i = 1; i < 16; i++) {
        if (rightDiagonalAscending) {
            const Position newPosition{static_cast<uint8_t>(position.x + i),
                                       static_cast<uint8_t>(position.y + i)};
            rightDiagonalAscending =
                CheckObstacle(validMoves, myPiece, newPosition, true);
            count++;
        }
        if (leftDiagonalDescending) {
            const Position newPosition{static_cast<uint8_t>(position.x - i),
                                       static_cast<uint8_t>(position.y - i)};
            leftDiagonalDescending =
                CheckObstacle(validMoves, myPiece, newPosition, true);
            count++;
        }
...
const std::vector<Position>& Engine::ValidMovesPawn(const Position& position) {
    ...
    auto checkOpposingPiece = [&](const Position& newPosition) {
        if (newPosition.x >= m_Board.size() ||
            newPosition.y >= m_Board.size()) {
            return;
        }
        ...
        if (IsValidPiece(opposingPiece.type) &&
            ...
            m_ValidMoves.push_back(
                newPosition);  // find a way to return the valid move instead of mutating the class object
        }
    };

    auto checkCanTake = [&](bool descending) {
        if (!descending) {
            const Position newPosLeft{static_cast<uint8_t>(position.x - 1),
                                      static_cast<uint8_t>(position.y + 1)};
            const Position newPosRight{static_cast<uint8_t>(position.x + 1),
                                       static_cast<uint8_t>(position.y + 1)};
    ...
    if (myPiece.color == PieceColor_White) {
        ...
        if (position.y == 6) {
            ...
            if (position.y >= 1) {
                const Position newPos{position.x,
                                      static_cast<uint8_t>(position.y - 1)};
...
const std::vector<Position>& Engine::ValidMovesKnight(
    const Position& position) {
    const auto& myPiece = m_Board[position.x][position.y];

    const Position topRight_1{static_cast<uint8_t>(position.x + 2),
                              static_cast<uint8_t>(position.y - 1)};
    const Position topRight_2{static_cast<uint8_t>(position.x + 1),
                              static_cast<uint8_t>(position.y - 2)};
    const Position bottRight_1{static_cast<uint8_t>(position.x + 2),
                               static_cast<uint8_t>(position.y + 1)};
    const Position bottRight_2{static_cast<uint8_t>(position.x + 1),
                               static_cast<uint8_t>(position.y + 2)};
    const Position topLeft_1{static_cast<uint8_t>(position.x - 2),
                             static_cast<uint8_t>(position.y - 1)};
    const Position topLeft_2{static_cast<uint8_t>(position.x - 1),
                             static_cast<uint8_t>(position.y - 2)};
    const Position bottLeft_1{static_cast<uint8_t>(position.x - 2),
                              static_cast<uint8_t>(position.y + 1)};
    const Position bottLeft_2{static_cast<uint8_t>(position.x - 1),
                              static_cast<uint8_t>(position.y + 2)};
...
const std::vector<Position>& Engine::ValidMovesBishop(
    const Position& position) {
